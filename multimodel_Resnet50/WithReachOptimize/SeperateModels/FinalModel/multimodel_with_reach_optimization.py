...
# ---------------------- Configuration ----------------------
CSV_PATH = r"E:\FinalData\6-images_tags_reach.csv"
IMG_PATH = r"E:\FinalData\selected_images"
...
OUTPUT_CSV = r"E:\FinalData\evaluation_output.csv"
TOP_K = 6
IMG_SIZE = 128
BATCH_SIZE = 128
EPOCHS = 8
...
def train_model_a(train_df, val_df, mlb, num_tags):
    ...
    print("Loss curve displayed and saved as model_a_loss_curve.png")
...
def plot_evaluation_metrics(metrics):
    ...
    plt.figure(figsize=(8, 6))
    bars = plt.barh(keys, values, color='steelblue')
    plt.xlim(0, 1)
    plt.title('Evaluation Metrics')
    plt.xlabel('Score')
    for bar in bars:
        plt.text(bar.get_width() + 0.01, bar.get_y() + 0.3, f"{bar.get_width():.4f}", fontsize=10)
    plt.grid(axis='x', linestyle='--', alpha=0.5)
    plt.tight_layout()
    plt.show()


def evaluate(model_a, model_b, val_df, TAGS, num_tags, follower_scaler, hashtag_count_scaler, ihc_scaler, mlb):
    device = DEVICE
    model_a.eval()
    model_b.eval()
    results = []
    ...
    for _, row in tqdm(val_df.iterrows(), total=len(val_df), desc="🔍 Evaluating"):
        ...
        with torch.no_grad():
            ...
            probs = torch.sigmoid(model_a(img_tensor, user_vector))[0].cpu().numpy()
            pred_labels = (probs >= 0.5).astype(int)

            # For metrics
            true_labels = np.zeros(num_tags)
            true_labels[mlb.transform([filtered_tags])[0] == 1] = 1
            y_true_list.append(true_labels)
            y_pred_list.append(pred_labels)
            y_probs_list.append(probs)

            # Get top-K predictions
            topk_indices = torch.topk(torch.tensor(probs), TOP_K).indices.cpu().tolist()
    ...
    from sklearn.metrics import f1_score, precision_score, recall_score, accuracy_score, hamming_loss, average_precision_score
...
def main():
    ...
    evaluate(model_a, ihc_model, val_df, TAGS, NUM_TAGS, follower_scaler, hashtag_count_scaler, ihc_scaler, mlb)
